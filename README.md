...
- 완료 핵심: GAS 데미지 파이프라인, 무기 3슬롯/히트스캔·근접, HUD(체력/탄약) 이벤트 갱신, 로컬 플레이어 전용 크로스헤어 표시, 사망 기본 로직, 탄피(`AmmoEject`) 배출
...
1. **에임 정렬:** 카메라 조준점 기준 총구 방향/탄도 일치 개선
...
5. **애니·동기화 보강:** 발사/장전/무기 전환의 원격 시각 일관성 점검
6. **디버그 정리:** 개발용 충돌 디버그 토글/표현 정리(출시 빌드 분리 기준 포함)
